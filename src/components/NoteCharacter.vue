<template>
    <div>
        ...
        >
          ...
          <button v-if="spaStore.currentCharacter.notes.length > 1" type="button" @click="deleteItem();" class="edit-item">
  ...
  const deleteItem = () => {
    ...
    spaStore.currentCharacter.notes.splice(props.itemIndex, 1);
    spaStore.charUpdate()
  }
  ...
  const updateItemData = () => {
      ...
      if(!noteTextInput.value.value){
        ...
        message.value = false;
        editing.value = false;
        spaStore.currentCharacter.notes[props.itemIndex] = noteTextInput.value.value;
  ...
  onMounted(() => {
    if (!spaStore.currentCharacter.notes[props.itemIndex]){
